 ...
 */
function loadGroupList(courseInfoListJson) {
    ...
    for(let i = 0; i < courseInfoListData.length ; i++){
        let divGrid = generateCourse(courseInfoListData[i]);
        div.appendChild(divGrid);
    }
}

function getCourseForGroup() {
    let request = null;
    if(window.XMLHttpRequest)
        request = new XMLHttpRequest();
    else
    ...
    request.onreadystatechange = function () {
        if(request.readyState === 4){
            if(request.status >= 200 && request.status<300 || request.status === 304)
                loadGroupList(request.responseText);
        }
    }
}
...
function generateCourse(courseInfo) {
    ...
    aRight.setAttribute("href","javascript:addCourse("+courseInfo.courseId+")");
